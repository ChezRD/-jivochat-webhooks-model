...
namespace ChezRD\Jivochat\Webhooks\Model\Traits;
...
trait PopulateObjectViaArray
{
    ...
    protected function populateFieldData($field_name, $class_name, $data, $iterable = false, $allow_null = false ) {
        ...
        if ($iterable === false) {
            $this->__populateFieldData($field_name, $class_name, $data);
            return;
        } else {
            foreach ($data as $item) {
                if (!(is_array($item) || (is_a($item, $class_name)))) {
                    throw new InvalidArgumentException('Invalid data given.');
                }

                $this->__populateFieldData($field_name, $class_name, $item);
            }
            return;
        }
